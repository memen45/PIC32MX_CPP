...
void USBCheckCDCRequest(void)
{
    ...
    switch(SetupPkt.bRequest){
        ...
        case SET_CONTROL_LINE_STATE:
            ...
            if(control_signal_bitmap.DTE_PRESENT == 1) UART_DTR = USB_CDC_DTR_ACTIVE_LEVEL;
            else UART_DTR = (USB_CDC_DTR_ACTIVE_LEVEL ^ 1);
            #endif
            inPipes[0].info.bits.busy = 1;
            break;
        #endif
...
void putUSBUSART(uint8_t *data, uint8_t  length)
{
     ...
     */
    USBMaskInterrupts();
    if(cdc_trf_state == CDC_TX_READY) mUSBUSARTTxRam((uint8_t*)data, length);     // See cdc.h
    USBUnmaskInterrupts();
}//end putUSBUSART
...
void CDCTxService(void)
{
    ...
    if(cdc_trf_state == CDC_TX_BUSY_ZLP){
        ...
        if(cdc_mem_type == USB_EP0_ROM){            // Determine type of memory source
            while(i--) *pCDCDst.bRam++ = *pCDCSrc.bRom++;
        } else {
            while(i--) *pCDCDst.bRam++ = *pCDCSrc.bRam++;
        }
