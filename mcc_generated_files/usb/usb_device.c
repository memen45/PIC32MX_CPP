...
void USBDeviceInit(void)
{
    ...
    for(i = 0; i < (sizeof(BDT)/sizeof(BDT_ENTRY)); i++) BDT[i].Val = 0x00;
...
void USBDeviceTasks(void)
{
     ...
     */
    if(USBIdleIF && USBIdleIE) USBSuspend();
    ...
    if(USBSOFIF){
        ...
        if(USBSOFIE) USB_SOF_HANDLER(EVENT_SOF,0,1);
        USBClearInterruptFlag(USBSOFIFReg,USBSOFIFBitNum);
            ...
            if(USBStatusStageTimeoutCounter) USBStatusStageTimeoutCounter--;
    ...
    if(USBStallIF && USBStallIE) USBStallHandler();
    ...
    if(USBTransactionCompleteIE){
        for(i = 0; i < 4u; i++){    //Drain or deplete the USAT FIFO entries.  If the USB FIFO ever gets full, USB bandwidth
            ...
            if(!USBTransactionCompleteIF) break;
...
void USBEnableEndpoint(uint8_t ep, uint8_t options)
{
    ...
    if(options & USB_OUT_ENABLED) USBConfigureEndpoint(ep, OUT_FROM_HOST);
    if(options & USB_IN_ENABLED) USBConfigureEndpoint(ep, IN_TO_HOST);
...
USB_HANDLE USBTransferOnePacket(uint8_t ep,uint8_t dir,uint8_t* data,uint8_t len)
{
    ...
    if(dir != 0) handle = pBDTEntryIn[ep]; //point to the IN BDT of the specified endpoint
    else handle = pBDTEntryOut[ep]; //else point to the OUT BDT of the specified endpoint
...
void USBDeviceAttach(void)
{
    if(USBDeviceState != DETACHED_STATE || USB_BUS_SENSE != 1) return;
    //if we are in the detached state and bus sense set
...
void USBCtrlEPAllowDataStage(void)
{
    ...
    if(controlTransferState == CTRL_TRF_RX){ //(<setup><out><out>...<out><in>)
        ...
        if(SetupPkt.wLength < inPipes[0].wCount.Val) inPipes[0].wCount.Val = SetupPkt.wLength;
...
static void USBConfigureEndpoint(uint8_t EPNum, uint8_t direction)
{
    ...
    if(direction == OUT_FROM_HOST) pBDTEntryOut[EPNum] = handle;
    else pBDTEntryIn[EPNum] = handle;
...
static void USBCtrlEPServiceComplete(void)
{
    ...
    if(inPipes[0].info.bits.busy == 0){
        if(outPipes[0].info.bits.busy == 1){
            ...
            if(USBDeferOUTDataStagePackets == false) USBCtrlEPAllowDataStage();
        ...
        if(SetupPkt.DataDir == USB_SETUP_DEVICE_TO_HOST_BITFIELD){
             ...
             */
            if(USBDeferINDataStagePackets == false) USBCtrlEPAllowDataStage();
            ...
            USBStatusStageEnabledFlag2 = false;
            USBStatusStageEnabledFlag1 = false;
            if(USBDeferStatusStagePacket == false) USBCtrlEPAllowStatusStage();
...
static void USBCtrlTrfTxService(void)
{
    ...
    if(inPipes[0].wCount.Val < (uint8_t)USB_EP0_BUFF_SIZE){
        ...
        if(shortPacketStatus == SHORT_PKT_NOT_USED) shortPacketStatus = SHORT_PKT_PENDING;
        else if(shortPacketStatus == SHORT_PKT_PENDING) shortPacketStatus = SHORT_PKT_SENT;
    }
...
static void USBCtrlTrfRxService(void)
{
    ...
    if(byteToRead > outPipes[0].wCount.Val) byteToRead = outPipes[0].wCount.Val;
    ...
    if(outPipes[0].wCount.Val > 0){
        ...
        if(outPipes[0].pFunc != NULL) outPipes[0].pFunc(); //Call the user's callback function
...
static void USBStdGetDscHandler(void)
{
    if(SetupPkt.bmRequestType != 0x80) return;
...
static void USBStdGetStatusHandler(void)
{
    ...
    switch(SetupPkt.Recipient){
        case USB_SETUP_RECIPIENT_DEVICE_BITFIELD:
             ...
             */
            if(self_power == 1) CtrlTrfData[0]|=0x01; // self_power is defined in HardwareProfile.h
            if(RemoteWakeup == true) CtrlTrfData[0]|=0x02;
            break;
        ...
        case USB_SETUP_RECIPIENT_ENDPOINT_BITFIELD:
            ...
            if(SetupPkt.EPDir == 0) p = (BDT_ENTRY*)pBDTEntryOut[SetupPkt.EPNum];
            else p = (BDT_ENTRY*)pBDTEntryIn[SetupPkt.EPNum];
...
static void USBCtrlTrfInHandler(void)
{
    ...
    if(USBDeviceState == ADR_PENDING_STATE){
        ...
        if(U1ADDR != 0u) USBDeviceState=ADDRESS_STATE;
        else USBDeviceState=DEFAULT_STATE;
    }
    ...
    if(controlTransferState == CTRL_TRF_TX){
        ...
        //  they are done
        if(outPipes[0].info.bits.busy == 1){
            if(outPipes[0].pFunc != NULL) outPipes[0].pFunc();
            outPipes[0].info.bits.busy = 0;
        }
...
static void USBStdFeatureReqHandler(void)
{
    ...
    if((SetupPkt.bFeature == USB_FEATURE_DEVICE_REMOTE_WAKEUP)&&
       (SetupPkt.Recipient == USB_SETUP_RECIPIENT_DEVICE_BITFIELD)){
        inPipes[0].info.bits.busy = 1;
        if(SetupPkt.bRequest == USB_REQUEST_SET_FEATURE) RemoteWakeup = true;
        else RemoteWakeup = false;
    }
...
void USBIncrement1msInternalTimers(void)
{
    ...
    USB1msTickCount++;
    if(USBIsBusSuspended()) return;
    //not suspended, keep track up to 255ms
    if(USBTicksSinceSuspendEnd != 255) ++USBTicksSinceSuspendEnd;
}
