...
namespace Syskey {
    ...
    void lock( void ){
        ...
        if( _unlock_count ) _unlock_count--;            //dec counter
        if( _unlock_count == 0 ) Reg::val( SYSKEY_ADDR, 0 ); //if 0, lock
    ...
    void unlock( void )
    {
        ...
        if( ! _unlock_count ){                          //first time, unlock
            Reg::val( SYSKEY_ADDR, 0xAA996655 );
            Reg::val( SYSKEY_ADDR, 0x556699AA );
        }
        _unlock_count++;                                //inc unlock_count
